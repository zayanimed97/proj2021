...
// **************** ADD TO CART *******************
    ...
    public function add($item, $id, $size,$color, $keys, $values) {
        ...
        //     $item->price = $item->vendor_price;
        //     // dd($item);
        // }
        
        $storedItem = ['qty' => 0,'size_key' => 0, 'size_qty' =>  $item->size_qty,'size_price' => $item->size_price, 'size' => $item->size, 'color' => $item->color, 'stock' => $item->stock, 'price' => Auth::guard('web')->check() && Auth::user()->IsVendor() ? $item->vendor_price : $item->price, 'item' => $item, 'license' => '', 'dp' => '0','keys' => $keys, 'values' => $values];
        ...
        if (Auth::guard('web')->check() && Auth::user()->IsVendor()) {
            $storedItem['price'] = $item->vendor_price * $storedItem['qty'];
        }
        else {
